"""Module that provides functions to detect candlestick patterns with one candle.
"""
...
def hammer_(candle: np.ndarray, T: np.ndarray) -> bool:
    """Definition: candle with a small body of either color, lower shadow between 2 ,
    3 times the length of the body , a small or no upper shadow.
    Trend: down.
    Prediction: reversal.
    """
    O = candle[:, 0]
...
def takuri_line_(candle: np.ndarray, T: np.ndarray) -> bool:
    """Definition: small  candle of either color with no upper shadow , lower shadow
